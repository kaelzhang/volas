...
from volas import DataFrame as StockDataFrame
from .common import (
    get_tencent,
    to_fixed
)
...
def test_style_namespace_and_cdl_alias(stock):
    """style.bullish/.bearish and the `cdl` alias match; `cdl` is case-insensitive too."""
...
def test_ema(stock):
    ...
    assert abs(ema20.iloc[-1] - close) < close * 0.1
