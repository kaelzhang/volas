...
比如 1 分钟 K 线——每来一根就 append 到 `DataFrame` 后面，再把所有指标在整张表上从头重算
一遍，如此反复，去逼近实盘里真实流入的 OHLCV。这里头大量都是纯粹的浪费——同样的列在每一
步被推倒重算——拿几年的 1 分钟数据跑下来，单这一块重复劳动就能把一次回测拖上**好几个小
时**。我想试的每一个想法、想扫的每一组参数，都要再交一遍这份税——决定研究节奏的，成了工
具，而不是思路。

于是我不再缝缝补补，而是把整个底层数据系统从头重写，彻底抛弃 `pandas`。这一注押对了：当年
要 10–20 分钟的那一道数据处理，如今几秒钟就跑完；曾经把回测拖得没完没了的逐根重算，如今
几乎可以忽略不计。回测本身仍有实打实的活要干——策略逻辑、撮合成交、账目结算——但数据层
已经不再是我枯坐干等的那一环。**volas** 就是这套数据层：在最要紧的地方快了数百倍，终于快
到可以让工具彻底退到一边。
